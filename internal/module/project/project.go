...
func CreateProject(c *gin.Context) {
	...
	// 查询项目是否已存在
	err := database.DB.Where("name = ? AND startdate = ?", req.Name, req.StartDate).First(&existingproject).Error
	if err == nil {
		// 项目已存在
		log.Warn("项目已存在", "name", req.Name, "startdate", req.StartDate)
		response.Fail(c, response.ErrAlreadyExists.WithTips("项目已存在"))
		return
	}
	if !errors.Is(err, gorm.ErrRecordNotFound) {
		// 数据库错误
		log.Error("数据库查询失败", "error", err, "name", req.Name, "startdate", req.StartDate)
